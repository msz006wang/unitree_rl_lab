...
"""Configuration for GO2W velocity tracking environment with wheel-legged training parameters."""
...
from unitree_rl_lab.assets.robots.unitree import UNITREE_GO2W_CFG as ROBOT_CFG
...
class RobotSceneCfg(InteractiveSceneCfg):
    """Configuration for the terrain scene with GO2W robot."""
...
class EventCfg:
    ...
    randomize_rigid_body_inertia = EventTerm(
        ...
        params={
            "asset_cfg": SceneEntityCfg("robot", body_names=".*"),
            "inertia_distribution_params": (0.8, 1.2),  # 缩放到80%-120%
            "operation": "scale",
            "distribution": "uniform",
        },
    )
...
class RewardsCfg:
    ...
    # 新增：动作镜像和同步奖励（基于msz006_go2w项目）
    # 鼓励对称和协调的运动，初始权重为0，可根据需要调整
    action_mirror = RewTerm(
        func=mdp.action_mirror,
        weight=0.0,  # 建议值: -0.01 到 -0.1
        params={
            "asset_cfg": SceneEntityCfg("robot"),
            "mirror_joints": [
                ["FR_(hip|thigh|calf).*", "RL_(hip|thigh|calf).*"],
...
class RobotEnvCfg(ManagerBasedRLEnvCfg):
    ...
    def __post_init__(self):
        ...
        self.actions.joint_pos.scale = {
            ".*_hip_joint": 0.125,  # Hip joints get smaller scale
            "^(?!.*_hip_joint).*": 0.25,  # Other leg joints get larger scale
        }
        self.actions.joint_vel.scale = 5.0  # Wheels use velocity control with scale 5.0
...
class RobotFlatEnvCfg(RobotEnvCfg):
    ...
    def __post_init__(self):
        ...
        print("✅ 使用平地训练配置 (RobotFlatEnvCfg)")
